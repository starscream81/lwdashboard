...
export default function ResearchPage() {
  ...
  // Aggregate per category progress
  const categoryProgress = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      entry.current += r.currentLevel ?? 0;
      entry.max += r.maxLevel ?? 0;
      entry.tracked = entry.tracked || !!r.trackStatus;
  ...
  const filteredRows = useMemo(() => {
    ...
    // Sort by orderIndex, fallback to name
      ...
      <main className="min-h-screen flex items-center justify-center bg-slate-950">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-300">
            Please sign in to manage your research.
          </p>
        </div>
      </main>
    );
  ...
  async function handlePriorityToggle(row: ResearchTrackingDoc) {
    const isPriority = row.priority != null && row.priority > 0;
    const newValue = isPriority ? null : 1;
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 space-y-6">
        ...
        <section className="space-y-2">
          ...
          <p className="text-sm text-slate-300">
            In Progress shows what you are researching now. Priority marks
            tasks that are on deck once current ones finish. Tracked For
            Tile marks focus categories that count toward the dashboard
        ...
        <section className="rounded-xl bg-slate-900/80 border border-slate-800 px-4 py-3 space-y-3">
          ...
          <p className="text-xs text-slate-400">
            Each category shows your overall completion. Click a tile to
            view and edit that category below. Categories with a dot are
            included in the dashboard research progress tile.
          </p>
              ...
              {!loadingData && !selectedCategory && (
                <tr>
                  <td
                    ...
                    className="px-3 py-4 text-center text-slate-400"
                  >
                    Click a category tile above to view research tasks.
                  </td>
                </tr>
              )}
                ...
                filteredRows.length === 0 && (
                  <tr>
                    <td
                      ...
                      className="px-3 py-4 text-center text-slate-400"
                    >
                      No research entries for this category yet.
                    </td>
                  </tr>
                )}
                    ...
                    <tr key={row.id} className="border-t border-slate-800">
                      ...
                      <td className="px-3 py-2 text-center">
                        <input
                          type="checkbox"
                          checked={isPriority}
                          onChange={() => handlePriorityToggle(row)}
                        />
                      </td>
                      <td className="px-3 py-2 text-center">
                        <input
